...
namespace SC_GUI {
    ...
    inline void BeginFrame(Graphics* g) {
        ...
        if (!mainFont) {
            // Modern Fonts: Segoe UI Semibold for headers, Regular for body
            // We'll stick to Segoe UI as it's standard and clean.
    ...
    inline void DrawFilledRoundedRect(float x, float y, float w, float h, float r, Color color) {
        GraphicsPath* path = CreateRoundedRectPath(x, y, w, h, r * 2); // r is diameter for AddArc? No, width/height of rect.
        // wait, AddArc takes bounds of ellipse. So r*2 is simpler?
        // Actually common implementation: r is diameter. 
        // Let's use standard r as diameter for arcs:
        float d = r * 2;
        GraphicsPath p;
    ...
    inline bool TabButton(const std::string& id, const std::string& text, float x, float y, float w, float h, bool active, const std::string& icon = "") {
        ...
        // DrawStringA(text, contentX, y + h/2 - 8, txtColor, largeFont, leftFormat); // Center vertically manually relative to line height?
        // Use vCenterFormat
    ...
    inline bool SkinCard(const std::string& id, const std::string& name, const std::string& image_url, int rarity, float x, float y, float w, float h, bool selected) {
        ...
        if (thumbCache.find(name) != thumbCache.end() && thumbCache[name]) {
             ...
             // Placeholder: Subtle circle or gradient
             // LinearGradientBrush grad(PointF((REAL)x, (REAL)y), PointF((REAL)x, (REAL)(y + h)), Color(255, 40, 40, 40), Color(255, 30, 30, 30));
             // gfx->FillRectangle(&grad, (REAL)(x+20), (REAL)(y+20), (REAL)(w-40), (REAL)(imgAreaH-40));
             // Just empty is fine for clean look, or a loader spinner?
        }
    ...
    inline bool Slider(const std::string& id, float* value, float min, float max, float x, float y, float w, float h) {
        ...
        // DrawFilledRoundedRect(knobX - 2, knobY - 2, knobSize + 4, knobSize + 4, knobSize + 4, Color(50, currentTheme.accent.GetR(), currentTheme.accent.GetG(), currentTheme.accent.GetB()));
