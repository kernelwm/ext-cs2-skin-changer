...
// Simple input handling for search
void UpdateSearchInput() {
    // Very basic toggle input for MVP
    // In a real hack you'd hook WndProc
    // For now, let's just assume empty = clear
}
...
void RenderWeaponTab(float x, float y, float w, float h)
{
    ...
    for (int idx : filteredIndices) {
        ...
        if (SC_GUI::SkinCard("wskin_" + availableSkins[idx].name, availableSkins[idx].name, availableSkins[idx].image_url, availableSkins[idx].rarity, cX, cY, itemW, itemH, selected)) {
            // selectedSkinIndex = idx; // No longer needed as we rely on skinManager
            if (idx != 0) skinManager->AddSkin(availableSkins[idx]);
            else {
                // Handle selecting default/vanilla?
                // skinManager doesn't seem to have a 'RemoveSkin' or 'SetVanilla'.
                // Assuming AddSkin with Paint=0 works if Vanilla has Paint=0.
...
void RenderSettingsTab(float x, float y, float w, float h)
{
    ...
    // Navigation to Configs? Or just embed it here?
    // User asked for "Settings" to have it. Let's make Settings a split view or just scrollable.
    // For now, let's put Settings controls at top, Configs below.
...
void RenderGloveTab(float x, float y, float w, float h)
{
    ...
    // Strip " Gloves" from name for search? API names are like "Sport Gloves | Vice"
    // So "Sport Gloves" is a good search term.
    ...
    for(int i=0; i<(int)gloves.size(); i++) {
        ...
        if (SC_GUI::SkinCard("glove_" + std::to_string(i), gloves[i].name, gloves[i].image_url, gloves[i].rarity, drawX, drawY, cardW, cardH, isSelected)) {
            ...
            //skinManager->Gloves.name = gloves[i].name; // Glove_t name string? might need to add it to struct in skindb.h if not present.
            // Earlier I added name to Glove_t.
...
void MenuThread()
{
	...
	{
		OnFrame(); // This now blocks per frame in the message loop inside Render?
        // Wait, overlay::Render has the loop? 
        // No, `overlay::Render` processes ONE frame.
        // But our `overlay::Render` in `window.hpp` had a `while(PeekMessage)` loop.
        // We need to make sure we don't block the logic thread or sleep too much.
	}
}
